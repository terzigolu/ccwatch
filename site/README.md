...
3. Configure the build (Cloudflare's unified UI detects `wrangler.toml` at repo root):
   ...
   `pages_build_output_dir = "./site/dist"`. If `watchstatus.bar` is
...
5. Site is live at `https://watchstatus.bar/` (or your chosen subdomain).
...
- Add CNAME record pointing to `watchstatus.bar`
- HTTPS is automatic
